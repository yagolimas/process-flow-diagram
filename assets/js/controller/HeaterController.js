var countHeater = 1;

class HeaterController {

    constructor() {
        this.name = "Heater";
    }

    add(graph, evt) {

        let modal = new Modal();
        let content = modal.createModalUnit();
        let subContent = modal.createContent(content);
        modal.open();

        const $input = document.querySelector('#unit');
        const $buttonOk = document.querySelector("#btn-ok");
        const $buttonCancel = document.querySelector("#btn-cancel");

        let name = graphEditor.validateCellNamesSequence("", "heater");

        $input.value = name;
        //$input.focus();
        $input.select();

        $buttonOk.focus();

        self = this;

        $input.onkeyup = function (event) {
            event.preventDefault();
            if (event.keyCode === 13) {
                $buttonOk.click();
            }
        };

        $buttonOk.onkeyup = function (event) {
            event.preventDefault();
            if (event.keyCode === 13) {
                $buttonOk.click();
            }
        };

        $buttonCancel.onkeyup = function (event) {
            event.preventDefault();
            if (event.keyCode == 13) {
                $buttonCancel.click();
            }
        };

        $buttonOk.onclick = function () {

            let parent = graph.getDefaultParent();

            graph.getModel().beginUpdate();

            try {
                name = $input.value;
                let isValid = graphEditor.validateCellNames(name);

                if (!isValid) {
                    throw new NameInUseExcpetion("Name already in use");
                }
                else {
                    let heater = new Heater(name, parent);

                    var pt = graph.getPointForEvent(evt);
                    heater.geometry.x = pt.x;
                    heater.geometry.y = pt.y;

                    self.tagNamePosition(heater);
                    self.tagAllocationPosition(heater);

                    graph.setSelectionCell(heater);

                    modal.close();
                }
            }
            catch (e) {
                e.openModalAlert();
            }
            finally {
                graph.getModel().endUpdate();
                containerOpened.checkConnectivityModalCreate(null, false);
            }
        };

        $buttonCancel.onclick = function () {
            graphEditor.decrementCellNamesSequence("heater");
            modal.close();
        };

        const $buttonHelp = document.querySelector("#btn-help");

        $buttonHelp.onclick = function () {
            callbackObj.openHelpModal(32);
        };

        subContent.addEventListener('keydown', (event) => {
            //event.preventDefault();
            if (event.keyCode == 27) {
                subContent.querySelector("#btn-cancel").click();
            }
        });
    }

    tagNamePosition(heater) {
        // label
        let tagName = heater.children.filter(function (item) {
            if (item.type == "TagName")
                return item;
        });

        if (tagName[0] != null && tagName[0] != undefined) {
            let index = heater.children.indexOf(tagName[0])

            if (index != null && index != -1) {
                heater.children[index].geometry.height = 25;
                heater.children[index].geometry.x = heater.children[index].positionX;
                heater.children[index].geometry.y = heater.children[index].positionY;
                heater.children[index].source = heater;
            }
        }
    }

    tagAllocationPosition(heater) {
        // tag allocation
        let tagName = heater.children.filter(function (item) {
            if (item.type == "TagAllocation")
                return item;
        });

        if (tagName[0] != null && tagName[0] != undefined) {
            let index = heater.children.indexOf(tagName[0])

            if (index != null && index != -1) {
                heater.children[index].geometry.height = 25;
                heater.children[index].geometry.x = heater.children[index].positionX;
                heater.children[index].geometry.y = heater.children[index].positionY;

                heater.children[index].source = heater;
            }
        }
    }
}
